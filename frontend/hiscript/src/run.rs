...
mod tests {
    ...
    fn an_agent_ask_round_trips() {
        let world = ScriptWorld {
            ask: Box::new(|prompt| Box::pin(std::future::ready(Ok(format!("echo: {prompt}"))))),
            ..ScriptWorld::disconnected()
        };
        let outcome = run(
            r#"export default async function (himark) {
    ...
    fn runs_share_no_state() {
        ...
        let probe =
            "export default async function (himark) { himark.log(typeof globalThis.seed); }";
        assert_eq!(
