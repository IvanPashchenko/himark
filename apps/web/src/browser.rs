...
struct Anchor(*const Mutex<Shared>);
...
impl BrowserTransport {
    pub async fn connect(url: &str) -> Result<Self, String> {
        ...
        let anchor = Anchor(Arc::into_raw(Arc::clone(&shared)));
        ...
        let socket = unsafe {
            ...
            if socket <= 0 {
                drop(Arc::from_raw(anchor.0));
                return Err(format!("websocket create failed ({socket})"));
            }
            let user_data = anchor.0 as *mut std::ffi::c_void;
...
impl Drop for BrowserTransport {
    fn drop(&mut self) {
        unsafe {
            ...
            drop(Arc::from_raw(self.anchor.0));
        }
    }
}
